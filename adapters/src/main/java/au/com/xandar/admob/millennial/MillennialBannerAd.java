...
public class MillennialBannerAd implements CustomEventBanner {
    ...
    public void requestBannerAd(final CustomEventBannerListener mediationListener,
        ...
        if (CustomEventConsts.DEBUG) Log.d(TAG, "#requestBannerAd androidVersion=" + Build.VERSION.SDK_INT + " deviceIsJellyBeanOrGreater=" + deviceIsJellyBeanOrGreater);
        ...
        if (CustomEventConsts.DEBUG) Log.d(TAG, "#requestBannerAd targetSDK=" + activity.getApplicationInfo().targetSdkVersion + " targetSDKis17OrGreater=" + targetSDKIs17OrGreater);
        ...
        if (CustomEventConsts.DEBUG) Log.d(TAG, "#requestBannerAd MillennialSDKVersion=" + MMAdViewSDK.SDKVER + " versionIs4.6.0(OrLower)=" + millennialVersionIsFourPointSixPointZero);
        ...
        if (deviceIsJellyBeanOrGreater && targetSDKIs17OrGreater && millennialVersionIsFourPointSixPointZero) {
            if (CustomEventConsts.DEBUG) Log.d(TAG, "#requestBannerAd JellyBean or greater device - Millennial-4.6.0 doesn't handle that well - bailing now");
        ...
        adView.setListener(new MMAdView.MMAdListener() {
            ...
            public void MMAdCachingCompleted(MMAdView mmAdView, boolean b) {
                if (CustomEventConsts.DEBUG) Log.d(TAG, "#adCachingCompleted - nothing to do");
            }

            @Override
            public void MMAdReturned(MMAdView mmAdView) {
                // Hand back the wrapped view so we get the correct sizing and gravity
                if (CustomEventConsts.DEBUG) Log.d(TAG, "#adReturned height=" + mmAdView.getHeight() + " width=" + mmAdView.getWidth() + " layoutParams#width=" + mmAdView.getLayoutParams().width + " layoutParams#height=" + mmAdView.getLayoutParams().height);
                mediationListener.onReceivedAd(wrappedAdView);
            }

            @Override
            public void MMAdFailed(MMAdView mmAdView) {
                if (CustomEventConsts.DEBUG) Log.d(TAG, "#adFailed");
                mediationListener.onFailedToReceiveAd();
            }

            @Override
            public void MMAdClickedToOverlay(MMAdView mmAdView) {
                if (CustomEventConsts.DEBUG) Log.d(TAG, "#adClickedToOverlay");
            ...
            public void MMAdOverlayLaunched(MMAdView mmAdView) {
                if (CustomEventConsts.DEBUG) Log.d(TAG, "#adOverlayLaunched (automatically) - nothing to do");
            }

            @Override
            public void MMAdRequestIsCaching(MMAdView mmAdView) {
                if (CustomEventConsts.DEBUG) Log.d(TAG, "#adRequestIsCaching - nothing to do");
            }
        });
